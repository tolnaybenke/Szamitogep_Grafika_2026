...
void render_scene(const Scene* scene)
{
    ...
    for (int wall = 0; wall < 4; wall++) {
        glPushMatrix();
            glRotatef(wall * 90.0f, 0.0f, 1.0f, 0.0f);
...
void update_scene(Scene* scene, double dt) {
    if (scene->watering_timer > 0.0) {
        ...
        // Ha megy az öntözés, fokozatosan növeljük a sűrűséget
        if (scene->fog_density < 0.08f) {
            scene->fog_density += (float)dt * 0.01f;
        }
        ...
        if (scene->fog_density > 0.0f) {
            scene->fog_density -= (float)dt * 0.02f;
        }
